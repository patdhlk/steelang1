...
class DefNode
  def eval(context)
    method = SteelangMethod.new(params, body)
...
class ClassNode
  def eval(context)
    steelang_class = Constants[name] # Check if class is already defined

    unless steelang_class # Class doesn't exist yet
      steelang_class = SteelangClass.new
      Constants[name] = steelang_class # Define the class in the runtime
    end

    class_context = Context.new(steelang_class, steelang_class)
